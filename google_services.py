...
# ─── RBS BROWSER HELPER JOBS ────────────────────────────────────────────────
# Stored in Config so Railway Hira and the local Mac helper can coordinate.

def get_rbs_jobs(include_done=False) -> list:
    raw = get_config("rbs_jobs")
    if not raw:
        return []
    try:
        jobs = json.loads(raw)
    ...
    for job in jobs:
        ...
        if status in ("done", "failed", "cancelled") and not include_done:
            continue
        ...
        clean.append({
            "id": str(job.get("id", "")),
            "type": str(job.get("type", "rbs_availability_check")),
            "status": status,
            "date": str(job.get("date", "")).strip(),
            "from_period": str(job.get("from_period", "")).strip(),
            "till_period": str(job.get("till_period", "")).strip(),
            "resources": [str(r).strip() for r in resources if str(r).strip()],
            "purpose": str(job.get("purpose", "")).strip(),
            "notify_chat_id": str(job.get("notify_chat_id", "")).strip(),
            "created": str(job.get("created", "")).strip(),
            "claimed_at": str(job.get("claimed_at", "")).strip(),
            "completed_at": str(job.get("completed_at", "")).strip(),
            "reported_at": str(job.get("reported_at", "")).strip(),
            "result": job.get("result", {}),
            "error": str(job.get("error", "")).strip(),
        })
    return [job for job in clean if job["id"]]


def set_rbs_jobs(jobs: list):
    set_config("rbs_jobs", json.dumps(jobs, ensure_ascii=False))


def add_rbs_availability_job(
    booking_date: str,
    from_period: str,
    till_period: str,
    resources: list,
    ...
    jobs = get_rbs_jobs(include_done=True)
    numeric_ids = [int(j["id"]) for j in jobs if str(j.get("id", "")).isdigit()]
    job = {
        "id": str(max(numeric_ids) + 1 if numeric_ids else 1),
        "type": "rbs_availability_check",
        "status": "queued",
        "date": booking_date.strip(),
        "from_period": from_period.strip().upper(),
        "till_period": till_period.strip().upper(),
        "resources": [str(r).strip() for r in resources if str(r).strip()],
        "purpose": purpose.strip(),
        "notify_chat_id": str(notify_chat_id).strip(),
        "created": datetime.now(SGT).isoformat(),
        "claimed_at": "",
        "completed_at": "",
        "reported_at": "",
...
def claim_next_rbs_job() -> dict | None:
    jobs = get_rbs_jobs(include_done=True)
    for job in jobs:
        if job.get("type") == "rbs_availability_check" and job.get("status") == "queued":
            job["status"] = "running"
            job["claimed_at"] = datetime.now(SGT).isoformat()
            set_rbs_jobs(jobs)
            return job
    return None


def update_rbs_job(job_id: str, status: str, result: dict | None = None, error: str = "") -> dict | None:
    jobs = get_rbs_jobs(include_done=True)
    updated = None
    for job in jobs:
        ...
        if job["status"] in ("done", "failed", "cancelled"):
            job["completed_at"] = datetime.now(SGT).isoformat()
        updated = job
        break
    if updated:
        set_rbs_jobs(jobs)
...
def unreported_rbs_jobs() -> list:
    return [
        job for job in get_rbs_jobs(include_done=True)
        if job.get("status") in ("done", "failed") and job.get("notify_chat_id") and not job.get("reported_at")
    ]


def mark_rbs_job_reported(job_id: str):
    jobs = get_rbs_jobs(include_done=True)
    for job in jobs:
        if str(job.get("id")) == str(job_id):
            job["reported_at"] = datetime.now(SGT).isoformat()
            break
    set_rbs_jobs(jobs)
