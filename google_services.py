...
def clear_classops_open_non_submissions(class_name: str) -> dict:
    ledger = get_classops_ledger()
    classes = ledger.setdefault("classes", {})
    class_key = str(class_name or "").strip().upper()
    if not class_key:
    ...
    record = classes.get(class_key)
    if not isinstance(record, dict):
        return {"class_name": class_key, "cleared_assignment_count": 0, "cleared_non_submission_count": 0}
    ...
    for assignment in record.get("assignments", []) or []:
        if not isinstance(assignment, dict):
            continue
        non_submitted = assignment.get("non_submitted", []) or []
        if not non_submitted:
            continue
        cleared_assignments += 1
        cleared_students += len(non_submitted)
        assignment["non_submitted"] = []
        assignment["tracking_mode"] = assignment.get("tracking_mode") or "non_submission_list"
    ...
    return {
        "class_name": class_key,
        "cleared_assignment_count": cleared_assignments,
        "cleared_non_submission_count": cleared_students,
    }


def remove_classops_open_non_submission_assignments(class_name: str) -> dict:
    ledger = get_classops_ledger()
    classes = ledger.setdefault("classes", {})
    class_key = str(class_name or "").strip().upper()
    if not class_key:
    ...
    record = classes.get(class_key)
    if not isinstance(record, dict):
        return {"class_name": class_key, "deleted_count": 0, "deleted_non_submission_count": 0, "assignments": []}
    ...
    for assignment in assignments:
        ...
        if non_submitted:
            removed.append(assignment)
            removed_students += len(non_submitted)
            source_path = str(assignment.get("source_path") or "").strip()
            if source_path:
                current = overrides.get(source_path)
                if not isinstance(current, dict):
                    current = {"title": str(current or "").strip(), "hidden": False}
                current["no_submission_needed"] = True
                overrides[source_path] = current
        else:
            kept.append(assignment)
    ...
    return {
        "class_name": class_key,
        "deleted_count": len(removed),
        "deleted_non_submission_count": removed_students,
        "assignments": kept,
    }
