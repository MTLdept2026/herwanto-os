...
The PWA backend includes Railway memory guardrails: one active chat/upload by default, capped request/upload sizes, disk-spooled PDF/DOCX/PPTX extraction for heavier education files, a `/healthz` memory readout, and load shedding before the container reaches the OOM killer. Tune with `HIRA_WEB_CHAT_CONCURRENCY`, `HIRA_WEB_UPLOAD_CONCURRENCY`, `HIRA_WEB_MAX_UPLOAD_MB`, `HIRA_WEB_MAX_DOCUMENT_MB`, and `HIRA_WEB_MEMORY_REJECT_RATIO` only after watching Railway memory for a while.
