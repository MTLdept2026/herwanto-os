...
Model routing is OpenAI-only. `HIRA_LLM_PROVIDER` is kept as `openai` for status clarity, while `HIRA_QUICK_MODEL`, `HIRA_ROUTER_MODEL`, `HIRA_STRUCTURED_MODEL`, `HIRA_AGENTIC_MODEL`, and `HIRA_DEEP_MODEL` choose the OpenAI models for each route. By default H.I.R.A uses a cost-conscious split: `gpt-5.4` for everyday agentic work, `gpt-5.5` for deep/complex escalation, `gpt-5.4-mini` for quick and structured turns, and `gpt-5.4-nano` for routing. Pin explicit model env vars only when you want to override those defaults.
