...
OpenAI usage tracking is enabled by default with `HIRA_OPENAI_USAGE_TRACKING=1`. H.I.R.A records a privacy-light rolling estimate from Responses API usage fields: model, route tier, token counts, cached tokens, native tool counts, and estimated SGD using `HIRA_OPENAI_USAGE_SGD_PER_USD` for conversion from OpenAI's USD pricing. It does not store prompts or responses. The latest summary appears in `/api/admin/status` under `runtime.openai_usage`; `HIRA_OPENAI_USAGE_PERSIST=1` stores the rolling summary in Config so it survives restarts, throttled by `HIRA_OPENAI_USAGE_PERSIST_INTERVAL_SECONDS`.
