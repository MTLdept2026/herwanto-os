...
The PWA backend includes Railway Hobby guardrails: two active chats/uploads by default, a bounded upload queue, larger capped request/upload sizes, disk-spooled PDF/DOCX/PPTX extraction for heavier education files, a minimal public `/healthz`, browser security headers, bounded client state keys, and cleanup once RSS reaches `HIRA_WEB_MEMORY_GC_MB` (512 MB by default). Repeatedly failing Home providers are paused temporarily and their last good result is reused; tune this with `HIRA_HOME_PROVIDER_FAILURE_THRESHOLD` and `HIRA_HOME_PROVIDER_COOLDOWN_SECONDS`. Detailed runtime health lives behind `/api/admin/status` with the PWA token. Keep `uvicorn` at one worker unless all scheduler work stays in the separate worker service; multiple web workers can duplicate in-process jobs. Tune with `HIRA_FAST_CHAT_MODE`, `HIRA_QUICK_ROUTE_MAX_CHARS`, `HIRA_QUICK_REPLY_MAX_TOKENS`, `HIRA_WEB_CHAT_CONCURRENCY`, `HIRA_WEB_UPLOAD_CONCURRENCY`, `HIRA_WEB_UPLOAD_QUEUE_WORKERS`, `HIRA_WEB_MAX_UPLOAD_MB`, `HIRA_WEB_MAX_DOCUMENT_MB`, `HIRA_WEB_CHAT_MAX_TOKENS`, and `HIRA_WEB_MEMORY_REJECT_RATIO` after watching Railway memory and CPU metrics.

The Growth log remains public by default for backward compatibility. Set `HIRA_GROWTH_PUBLIC=0` to return `404` for the Growth page and its supporting assets on a private deployment.
...
Model routing is OpenAI-only. Normal PWA messages are interpreted and answered by the agentic model rather than local keyword shortcuts; deterministic execution is reserved for explicit slash commands and tightly scoped pending confirmations. `HIRA_LLM_PROVIDER` is kept as `openai` for status clarity, while `HIRA_QUICK_MODEL`, `HIRA_ROUTER_MODEL`, `HIRA_STRUCTURED_MODEL`, `HIRA_AGENTIC_MODEL`, and `HIRA_DEEP_MODEL` choose the OpenAI models for each route. The default agentic/deep model is the `gpt-5.6` alias (GPT-5.6 Sol); quick, router, and structured helpers remain on the smaller models for non-answer background work and fallbacks. Pin explicit model env vars only when you want to override those defaults.
...
OpenAI usage tracking is enabled by default with `HIRA_OPENAI_USAGE_TRACKING=1`. H.I.R.A records a privacy-light rolling estimate from OpenAI usage fields: model, route tier, token counts, cached tokens where reported, native OpenAI tool counts, and estimated SGD using the configured provider FX setting. It does not store prompts or responses. The active provider summary appears in `/api/admin/status` under `runtime.api_usage`, with details under `runtime.openai_usage`; persistence is controlled by `HIRA_OPENAI_USAGE_PERSIST`. `HIRA_OPENAI_MONTHLY_BUDGET_SGD` defaults to `20` and is enforced before every Responses API request using a conservative reservation; requests that could cross the ceiling are blocked. Ledgers are automatically scoped by `RAILWAY_SERVICE_NAME` (or `HIRA_OPENAI_USAGE_SCOPE`) so separate services cannot overwrite one another's accounting; allocate service caps so their sum equals the desired project ceiling. `HIRA_OPENAI_BUDGET_MIN_REQUEST_RESERVE_SGD` defaults to `0.05`. Keep a provider-side OpenAI project budget as the final billing backstop as well.
