...
3. Go to **Variables** tab → add the required env vars:
   ...
   - Optional DeepSeek trial: `HIRA_LLM_PROVIDER=deepseek`, `DEEPSEEK_API_KEY`, and keep `OPENAI_API_KEY` configured for OpenAI-only features
...
The PWA chat uses the same H.I.R.A tool brain as Telegram. With the same production env vars, it can create/delete calendar events, add/complete reminders and follow-ups, manage marking progress, read Gmail, create drafts, generate DOCX/PPTX artifacts, process uploaded documents/images/voice notes, remember context, use timetable context, fetch NEA weather, and fetch news when search is configured. The morning digest always merges configured watch topics with H.I.R.A's built-in live radar for Liverpool, F1/Mercedes, AI tools, Codex/Claude/Gemini/Kimi, Teenage Engineering, Android, iOS, Islam, Singapore education, Nothing products, and Nothing OS.
...
Digest social search is optional and bounded. By default `HIRA_DIGEST_SOCIAL_SEARCH=1` adds a capped X/Twitter-style web-search pass for the first few high-interest live topics using `HIRA_DIGEST_SOCIAL_DOMAINS=x.com,twitter.com` and `HIRA_DIGEST_SOCIAL_TOPIC_LIMIT=6`. This is not an authenticated X API integration; it uses normal search indexing, so treat social hits as leads and confirm important facts with official or reputable sources.

The PWA backend includes Railway Hobby guardrails: two active chats/uploads by default, a bounded upload queue, larger capped request/upload sizes, disk-spooled PDF/DOCX/PPTX extraction for heavier education files, a minimal public `/healthz`, browser security headers, bounded client state keys, and load shedding only under high memory pressure. Detailed runtime health lives behind `/api/admin/status` with the PWA token. Keep `uvicorn` at one worker unless all scheduler work stays in the separate worker service; multiple web workers can duplicate in-process jobs. Tune with `HIRA_FAST_CHAT_MODE`, `HIRA_QUICK_ROUTE_MAX_CHARS`, `HIRA_QUICK_REPLY_MAX_TOKENS`, `HIRA_WEB_CHAT_CONCURRENCY`, `HIRA_WEB_UPLOAD_CONCURRENCY`, `HIRA_WEB_UPLOAD_QUEUE_WORKERS`, `HIRA_WEB_MAX_UPLOAD_MB`, `HIRA_WEB_MAX_DOCUMENT_MB`, `HIRA_WEB_CHAT_MAX_TOKENS`, and `HIRA_WEB_MEMORY_REJECT_RATIO` after watching Railway memory and CPU metrics.

PWA sync troubleshooting note from 2026-05-27: if chat works but the home screen says it cannot sync services, first compare `/api/app/version` and `/api/home?days=7` on `hira-pwa`. On Railway, run a protected smoke from the PWA service env so the token is not printed:
```bash
railway run --service hira-pwa python3 -c 'import os,json,urllib.request,time; token=os.environ["HIRA_WEB_TOKEN"]; started=time.perf_counter(); req=urllib.request.Request("https://hira-pwa-production.up.railway.app/api/home?days=7",headers={"X-Hira-Token":token}); data=json.load(urllib.request.urlopen(req,timeout=35)); print(json.dumps({"wall_ms":round((time.perf_counter()-started)*1000),"services":data.get("services",{}),"sync_timings":data.get("sync_timings",[])},sort_keys=True))'
```
The May 2026 failure was a timeout, not missing env vars: `/api/home` was taking about 30-31s while the browser aborted at 28s. The fix was to keep home service status cheap, avoid fresh Google reads inside `_service_status`, reuse the already-fetched memory snapshot for relief/absence checks, and cache timetable week config for the home build. After deploying `dd63073` and `e6bfd67`, production `/api/home` dropped to about 11s. If this regresses, inspect `sync_timings`: `core_build` should be a few seconds, `extra.services` should be near 0ms, and slow optional panels should timeout without blocking beyond the home endpoint budget. If Work Gmail alone shows `Reconnect`, refresh `GOOGLE_WORK_GMAIL_REFRESH_TOKEN`; that is separate from the home sync timeout.
...
Model routing is configurable with `HIRA_LLM_PROVIDER`, `HIRA_QUICK_MODEL`, `HIRA_ROUTER_MODEL`, `HIRA_STRUCTURED_MODEL`, `HIRA_AGENTIC_MODEL`, and `HIRA_DEEP_MODEL`. Set `HIRA_LLM_PROVIDER=openai` to route H.I.R.A's core chat/tool calls through OpenAI. Set `HIRA_LLM_PROVIDER=deepseek` with `DEEPSEEK_API_KEY` to trial DeepSeek through its Anthropic-compatible endpoint while keeping `OPENAI_API_KEY` configured for voice transcription, realtime, vector sync, upload vision handoff, and deliberate rollback. By default, OpenAI mode uses a cost-conscious split: `gpt-5.4` for everyday agentic work, `gpt-5.5` for deep/complex escalation, `gpt-5.4-mini` for quick and structured turns, and `gpt-5.4-nano` for routing. DeepSeek mode defaults to `deepseek-v4-flash` for quick/agentic/structured/router work and `deepseek-v4-pro` for deep escalation; unknown DeepSeek model env values are rejected because DeepSeek's Anthropic API may otherwise silently map them back to Flash. DeepSeek thinking mode is explicit: `HIRA_DEEPSEEK_THINKING_MODE=auto` disables thinking for quick/classifier text calls and enables it for agentic/deep calls, using higher effort for deep work. The DeepSeek Anthropic-compatible path is treated as text/tool-only: image uploads are first extracted through `HIRA_OPENAI_VISION_MODEL` when `OPENAI_API_KEY` is configured, then handed back to DeepSeek as text/tool work; other unsupported image/document message blocks are blocked with an honest explanation instead of being sent to DeepSeek and failing opaquely. Set `HIRA_DEEPSEEK_OPENAI_FALLBACK=1` only when you want failed DeepSeek chat turns to fall back to OpenAI; it is off by default so DeepSeek trials do not get silently masked. Pin explicit model env vars only when you want to override those defaults.

OpenAI full-power mode uses the Responses API with stateful response IDs, reasoning controls, prompt caching, action preflight, and richer tracing. Paid native web search, code interpreter, realtime voice, and vector-store sync are opt-in cost levers. If you create an OpenAI vector store, set `HIRA_OPENAI_VECTOR_SYNC_ENABLED=1` along with `HIRA_OPENAI_FILE_SEARCH_VECTOR_STORE_IDS` or `HIRA_OPENAI_MEMORY_VECTOR_STORE_ID`; H.I.R.A will index assistant memory snapshots and uploaded-file summaries into that store for semantic recall. Native OpenAI citations/file-search/code-interpreter observations are folded into the same source-confidence trace used by H.I.R.A's local tools.

OpenAI usage tracking is enabled by default with `HIRA_OPENAI_USAGE_TRACKING=1`. DeepSeek usage tracking is enabled in DeepSeek mode with `HIRA_DEEPSEEK_USAGE_TRACKING=1`. H.I.R.A records a privacy-light rolling estimate from provider usage fields: model, route tier, token counts, cached tokens where reported, native OpenAI tool counts, and estimated SGD using the configured provider FX setting. It does not store prompts or responses. The active provider summary appears in `/api/admin/status` under `runtime.api_usage`, with provider-specific details under `runtime.openai_usage` and `runtime.deepseek_usage`; persistence is controlled by `HIRA_OPENAI_USAGE_PERSIST` and `HIRA_DEEPSEEK_USAGE_PERSIST`.

State-changing chat turns use `HIRA_OPENAI_ACTION_PLANNER=1` by default in OpenAI mode. This adds a strict structured-output preflight before side-effecting actions such as calendar writes, reminders, nudges, Gmail drafts, memory writes, classlist edits, and artifact generation. If a date, recipient, target item, class/student/column, or other safety-critical detail is missing, H.I.R.A asks only for that detail instead of guessing. Set `HIRA_OPENAI_REALTIME_ENABLED=1` to enable realtime voice with `HIRA_OPENAI_REALTIME_MODEL=gpt-realtime-2`.

To create the memory vector store with the configured OpenAI key, run `python3 scripts/bootstrap_openai_vector_store.py`, then copy the printed `HIRA_OPENAI_MEMORY_VECTOR_STORE_ID` and `HIRA_OPENAI_FILE_SEARCH_VECTOR_STORE_IDS` into Railway.
...
PWA chat also keeps a short-lived working memory per client for the current subject, latest correction, pending action, and competing older subjects. This protects follow-up turns like "that day" or "remind me again" from drifting back to an older named event. State-changing tools such as Calendar events, reminders, nudges, check-ins, and follow-ups run through a lightweight validation/audit step before saving; unresolved vague references are blocked and the tool result includes an internal action audit for review.
