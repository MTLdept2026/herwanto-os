...
Long-term memory is bucketed into `profile`, `preferences`, `people`, `places`, `teaching`, `business`, `projects`, `sports`, `files`, `templates`, `constraints`, `recent_summaries`, `topic_profiles`, `correction_ledger`, and `self_reflections`. Use `/remember category | note` or the `remember_user_info` tool to store stable context in the right bucket. When Herwanto says he has a new interest or is getting into a topic, `create_topic_profile` stores a structured profile with what to track, preferred angle, live facts to web-check, stable background context, and update cadence. When he corrects H.I.R.A or flags an important learning moment, chat handling writes a high-priority correction and/or self-reflection note so future answers can improve from the event.
